...
export default function CheckinPage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (response.status === 401) {
      window.location.href = "/login?redirectedFrom=/checkin";
      return;
    }
    if (!response.ok) {
    ...
    <>
      ...
      <main className="grid min-h-screen place-items-center px-4 py-32">
        <section className="glass w-full max-w-2xl rounded-[2rem] p-6 md:p-9">
          <ScanLine className="h-12 w-12 text-cyan" />
          ...
          <form onSubmit={submit} className="mt-8 space-y-4">
            <label className="block">
              ...
              <input className="input-cyber text-center text-2xl font-black uppercase tracking-[0.18em]" value={code} onChange={(event) => setCode(event.target.value)} placeholder="AIOT-XXXXXX" />
            </label>
